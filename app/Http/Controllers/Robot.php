...
class Robot extends Controller
{
    ...
    public function cekCode($workflow_id){
        $get = Model::where('workflow_id', $workflow_id)->first();
        if (!$get) {
            return response()->json([
                'message' => 'Workflow ID tidak ditemukan.',
    ...
    public function message(Request $request){
        ...
        if (!$get) {
            ...
            if($request->query('continue') != null){
                ...
                RobotLog::where('robot_id', $get->id)->where('status', 0)->update([
        ...
        if (!$get) {
            ...
            if($getLog->count() > 0){
                ...
                if($request->query('status') == 2){
                    RobotLog::where('robot_id', $get->id)->where('code', $getLog->code)->update([
                        'end_date' => date('Y-m-d H:i:s'),
                        'status' => 1 // Status 1 yang berarti 'berhasil'
                    ]);
    ...
    public function run($workflow_id){
        ...
        date_default_timezone_set('Asia/Jakarta');
          ...
          if($get->status == 2){
            return response()->json([
                'success' => 'refresh',
                'message' => 'Robot sudah dijalankan!',
                'id' => $get->id
            ]);
          }
          $robot_id = $get->id;
    ...
    public function getLogDetail($workflow_id)
    {
        ...
        if (!$get) {
            ...
            $cek_value = RobotLogDetail::join('robot_logs as rl', 'rl.id', '=', 'robot_log_details.robot_log_id')->where('rl.robot_id', $get->id)->where('rl.status', 0)->select('robot_log_details.nomor')->orderBy('robot_log_details.nomor','desc')->first();
            ...
            $return['value_percentase'] = $cek_value;
