...
class Logs extends AbstractTable
{
    ...
    public function for()
    {
        ...
        if(session()->get('client_id') > 0){
            ...
            $query = QueryBuilder::for(Model::class)
                ->select('robot_log_details.*', 'rl.code', 'r.name', 'robot_log_details.start_date as dimulai')
                ...
                ->whereNot('r.id', 86)
                ->defaultSort('-rl.code')
                ->defaultSort('-robot_log_details.start_date')
    ...
    public function configure(SpladeTable $table)
    {
        $table
            ...
            ->column('start_date', 'Waktu Selesai', sortable: false)
