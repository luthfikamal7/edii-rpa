...
class User extends Controller
{
    ...
    public function access(Model $user){
        ...
        $robot = Robot::leftJoin('user_robots as ur', 'ur.robot_id', '=', 'robots.id')->where('ur.user_id', $user->id)->select('robots.*', 'ur.user_id', 'ur.status as status_robot');
        if($robot->count() == 0){
            $robot = Robot::leftJoin('user_robots as ur', 'ur.robot_id', '=', 'robots.id')->select('robots.*', 'ur.user_id', 'ur.status as status_robot');
        }
